import 'package:chaty_talk/firebase/local_db/db/hive_methods.dart';
import 'package:chaty_talk/firebase/local_db/db/sqlite_methods.dart';
import 'package:chaty_talk/models/log.dart';
import 'package:flutter/cupertino.dart';

class LogRepository {
  static var dbObject;
  static bool isHive;

  static init({@required bool isHive, @required String dbName}) {
    dbObject = isHive ? HiveMethods() : SqliteMethods();
    dbObject.openDb(dbName);
    dbObject.init();
  }

  static addLog(LogModel logModel) => dbObject.addLog(logModel);

  static deleteLogs(int logId) => dbObject.deleteLogs(logId);

  static getLogs() => dbObject.getLogs();

  static close() => dbObject.close();
}
